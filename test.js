...
// })
// 6.Створити кілька елементів,
//     і при кліку на них додавати йому клас для анімації 
//     (можна використовувати CSS - клас із анімацією).
// const listEl = document.getElementById("list");
// listEl.addEventListener("click", e => {
//    //context = this
//    listEl.classList.toggle("text-blue-500")
// })
...
// let setRandomColorButton = document.getElementById('button');
// let randomIndex;

// setRandomColorButton.addEventListener('click', () => {
//    let childrenList = listEl.children; //всі дочірні елементи
//    let childrenCount = childrenList.length//кількість дочірніх елементів
//    randomIndex = Math.floor(0 + Math.random() * (childrenCount + 1 - 0));

//    for (let i = 0; i < childrenCount; i++) {
//       if (childrenList[i].className.includes('text-blue-500')) {
//          childrenList[i].classList.remove('text-blue-500')
//       }
//       if (i === randomIndex) {
//          childrenList[i].classList.add('text-blue-500')
//       }
//    }
// })
// 7. Лічильник кліків: Створити кнопку, при натисканні на яку збільшується лічильник на сторінці. 
// Показувати значення лічильника у реальному часі.
// const buttonEl = document.getElementById("button");
...
// 8. Фільтрація списку: Створити інпут та список елементів. 
// При введенні тексту в інпут фільтрувати список, показуючи лише ті елементи,
//     що містять введений текст.
const productList = [
   { title: "apple", id: "1", },
   { title: "peach", id: "2", },
   { title: "banana", id: "3", },
...
let parentList = document.getElementById('prod_list');//отримали посилання на елемент у який будемо додавати елементи товарів
let productInput = document.getElementById('inputEl');//посилання на інпут
let filteredProducts = [];//масив для фільтрованих елементів

//код який генерує список товарів
productList.forEach(({title, id})=>{
   let listEl = document.createElement("li");//створюємо елемент
   listEl.textContent = title;//змінюємо текст елементу
   listEl.id = id;//змінюємо айді елементу
   parentList.append(listEl);//поміщаємо елемент у parent
});


productInput.addEventListener('change',(event)=>{
   let filterValue = event.target.value;//значення з інпуту

   filteredProducts = productList.filter(product=>{ //шукаємо товари у title яких міститься filterValue
      if(product.title.includes(filterValue)) return product
   });

   parentList.replaceChildren(); //видаляємо всі елементи зі списку

   //генеруємо фільтрований список
   filteredProducts.forEach(({title, id})=>{
      let listEl = document.createElement("li");//створюємо елемент
      listEl.textContent = title;//змінюємо текст елементу
      listEl.id = id;//змінюємо айді елементу
      parentList.append(listEl);//поміщаємо елемент у parent
...
// 7. Лічильник кліків:
//  Створити кнопку, при натисканні на яку збільшується лічильник на
// сторінці.Показувати значення лічильника у реальному часі.
